    ...
    : QWidget(parent)
{
    ...
    m_colors[7] = QColor(255, 255, 255);
    m_colors[8] = QColor(100, 200, 100);
    m_colors[9] = QColor(200, 100, 200);
    m_colors[10] = QColor(100, 100, 255);
    m_colors[11] = QColor(255, 120, 80);
    m_colors[12] = QColor(80, 200, 200);
    m_colors[13] = QColor(200, 200, 100);
    m_colors[14] = QColor(180, 100, 140);
    m_colors[15] = QColor(100, 100, 100);
    ...
    m_singleW = SIDEBAR_X + 160 + BOARD_OFFSET_X;
    m_singleH = BOARD_OFFSET_Y * 2 + CELL_SIZE * TetrisBoard::HEIGHT;
    setFixedSize(m_singleW, m_singleH);
    setWindowTitle("Tetris");

    m_menuScreen = new MenuScreen(this);
    m_menuScreen->resize(m_singleW, m_singleH);
    m_menuScreen->show();
    m_menuScreen->setFocus();
    connect(m_menuScreen, &MenuScreen::singlePlayerRequested, this, &TetrisWindow::startSinglePlayer);
    connect(m_menuScreen, &MenuScreen::twoPlayerRequested, this, &TetrisWindow::startTwoPlayer);
    connect(m_menuScreen, &MenuScreen::aiModeRequested, this, &TetrisWindow::startAiMode);
    connect(m_menuScreen, &MenuScreen::quitRequested, this, &TetrisWindow::quitGame);

    setFocusPolicy(Qt::StrongFocus);
}

void TetrisWindow::startSinglePlayer()
{
    m_menuScreen->hide();
    setFocus();
    m_board->newGame();
...
void TetrisWindow::startTwoPlayer()
{
    m_menuScreen->hide();

    if (!m_dualGame) {
        m_dualGame = new DualGame(this, m_colors, m_colorsLight, m_colorsDark);
        connect(m_dualGame, &DualGame::backToMenu, this, &TetrisWindow::backFromDual);
    }

    QSize size = DualGame::windowSize();
    m_menuScreen->resize(size.width(), size.height());
    setFixedSize(size.width(), size.height());
    m_dualGame->resize(size.width(), size.height());
    m_dualGame->show();
    m_dualGame->start();
}

void TetrisWindow::backFromDual()
{
    m_dualGame->hide();
    setFixedSize(m_singleW, m_singleH);
    m_menuScreen->resize(m_singleW, m_singleH);
...
void TetrisWindow::startAiMode()
{
    m_menuScreen->hide();

    if (!m_vsAiGame) {
        m_vsAiGame = new VsAiGame(this, m_colors, m_colorsLight, m_colorsDark);
        connect(m_vsAiGame, &VsAiGame::backToMenu, this, &TetrisWindow::backFromAi);
    }

    QSize size = VsAiGame::windowSize();
    m_menuScreen->resize(size.width(), size.height());
    setFixedSize(size.width(), size.height());
    m_vsAiGame->resize(size.width(), size.height());
    m_vsAiGame->show();
    m_vsAiGame->start();
}

void TetrisWindow::backFromAi()
{
    m_vsAiGame->hide();
    setFixedSize(m_singleW, m_singleH);
    m_menuScreen->resize(m_singleW, m_singleH);
...
void TetrisWindow::drawSidebar(QPainter &painter)
{
    ...
    painter.drawText(sx, sy, "控制:");
    sy += 20;
    QFont smallFont("Consolas", 9);
    painter.setFont(smallFont);
    painter.drawText(sx, sy, "← →  移动");
    sy += 18;
    painter.drawText(sx, sy, "↑     旋转");
    sy += 18;
    painter.drawText(sx, sy, "↓     缓降");
    sy += 18;
    painter.drawText(sx, sy, "Space 快速下落");
    sy += 18;
    painter.drawText(sx, sy, "P     暂停");
    sy += 18;
    painter.drawText(sx, sy, "R     重新开始");
    sy += 18;
    painter.drawText(sx, sy, "esc     退出");
